...
- [ ] REQ-001: ブックマークの作成・復元に失敗しても、DBに保存済みの楽曲一覧を表示する。
- [ ] REQ-002: フォルダへのアクセス権を復元できない場合、その旨を区別して通知する。
- [ ] REQ-003: フォルダ選択・ブックマーク作成の失敗をユーザーに提示し、未処理例外にしない。
- [ ] REQ-004: プラットフォーム実装が存在しない環境（Windows、テスト）では従来どおり無効化する。

## 5. 完了条件

- [ ] `PlatformException` を捕捉し、ライブラリ読み込みを継続できる。
- [ ] アクセス権喪失時の警告を一覧画面に表示する。
- [ ] `chooseAndScanFolder` の例外がエラー表示になる。
- [ ] ブックマーク失敗時の回帰テストが追加され、成功する。
