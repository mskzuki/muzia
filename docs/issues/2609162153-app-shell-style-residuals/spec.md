...
- 確認事項1（T1）: チェブロンは見送り。タイトルはサイドバー幅の右（コンテンツ列上）へ移す。
- 確認事項2（T2）: ⋯ボタンは見送り。
- 確認事項3（T3）: 桁区切りと fgTertiary のみ適合。合計サイズ表示はファイルサイズを
  保持していないため見送り（別途判断）。
- 確認事項4（L2）: ケバブ列は常設とし、右クリックと同じコンテキストメニューを開く。
- 確認事項5（P2）: 2609021649 の実装時に調整（本課題では未着手）。
- 補足: L1 / L3 / D3 のためにトークン `rowDivider`（gray-a2）・`accentBorder`（indigo-a5）・
  `warnBorder`（amber-a5）を追加した。D2 / D3 は本課題で対応済み（2609021650 / 1651 では不要）。
